...
which is not incidental. This experiment reuses that experiment's DCDHF-Me2
numbers as the top rung of its DCDHF ladder, so the two have to have been
computed under the same software, and a diff of the two exports is the cheapest
way for a reader to confirm it.
...
constructs the benzylidenemalononitrile ladder analytically from standard bond
lengths and angles rather than generating and force-field-optimizing it, and it
is the same constraint `research/dcdhf-me2-transitions` recorded when it built
its benzene starting structure by hand.
